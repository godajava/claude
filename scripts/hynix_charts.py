...
사용법:
    python3 scripts/hynix_charts.py <data.json> <출력_디렉터리>

data.json 형식은 reports/sk-hynix/assets/ 아래 기존 파일을 참고.
생성 차트: price_trend / target_prices / quarterly_earnings / hbm_share
색상은 dataviz 검증을 통과한 팔레트 고정값 (light 서피스 기준).
"""
import json
import sys
import os

# 팔레트 (validate_palette.js 통과: #2a78d6,#008300,#e87ba4 / light)
SURFACE = "#fcfcfb"
INK = "#0b0b0b"
INK2 = "#52514e"
MUTED = "#898781"
...
def main():
    ...
    for fn in (price_trend, target_prices, quarterly_earnings, hbm_share):
        if fn.__name__ in d:
            fn(d, out)
